...
def plotLocations(BSLoc, nodeLoc, maxX, maxY, bestDist, distMatrix):
    ...
    # legend_elements = [Line2D([0], [0],marker='o', color='w', label='Normal end-device'     , markerfacecolor='g',
    # markers=15), Line2D([0], [0],marker='s', color='w', label='Inteligent end-device' , markerfacecolor='b',
    # markers=15), Line2D([0], [0],marker='^', color='w', label='Gateway'               , markerfacecolor='c',
    # markers=15)]
    for b in BSLoc[:, 1:3]:
    ...
    for i in range(0, len(nodeLoc)):
        if nodeLoc[i, 13] == 0:
            nodePoints = loc_plot.plot(nodeLoc[i, 1], nodeLoc[i, 2], "ro", mfc='none',
                                       label="Normal end-device")  # , label="normal node")
        else:
            if (i == 300):
                print("Here I was")
                nodePoints = loc_plot.plot(nodeLoc[i, 1], nodeLoc[i, 2], "ro", mfc='none', color='blue',
                                           label="Inteligent end-device")
            else:
                nodePoints = loc_plot.plot(nodeLoc[i, 1], nodeLoc[i, 2], "ro", mfc='none',
                                           label="Inteligent end-device")  # , label="intelligent node")
    for i in range(0, len(nodeLoc)):
        if (i == 300):
    ...
    # plt.tight_layout()
    plt.savefig('CompetitionLayout.png', format='png', bbox_inches='tight')
    print('Done')
    # plt.show() #############################################################
